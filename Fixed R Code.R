...
convert <- function(data, converters) {
  ...
  for (name in names(converted)) {
    ...
    unconverted <- c(select(data, name))[which(converted[[name]]==-1)] #rewritten by PH
...
converters <- list(
  ...
  `MONTH_f` = converter.numeric(c(1, 2, 3, 5, 6, 7, 8, 9, 11, 12), has_na = FALSE),
...
#This code converts the data into a predictable version
x_convert <- x %>% convert(converters) %>% as_tibble()

#this takes the converted data and creates a predicted version 
pred <- x_convert %>% predict()

#pred is a vector of predicted values
#let's check some dimensions
dim(x)
dim(x_convert) #note that we now only have 4 columns (4 vars in model)
length(pred) #produces one prediction for each record

## Create a histogram of Predictions
hist(pred, main = "Histogram of Predictions", col = "orange3")
plot(density(na.omit(pred))$x, density(na.omit(pred))$y, type = "l", main = "Density Plot")

## Create a lift curve
#first we bin the predicted values into 10 buckets
pred_cut <- cut(pred, 10)
#now we create a table of success/failure by pred_cut and display by proportions rather than counts
table(pred_cut, x$DELAYED) %>% prop.table()

#now we make a mosaic plot to display counts
par(las = 2)
table(pred_cut, x$DELAYED) %>% prop.table() %>% mosaicplot(color = c("orange3","dodgerblue3"), cex = .5)
title("LIft Curve")

#alternate lift curve that might look better

props <- table(pred_cut, x$DELAYED) %>% prop.table()
#calculate proportion IN EACH BIN
prop_success_in_bin <- props[,2]/(props[,1] + props[,2]) #maybe not the best variable name here


#The following code creates a plot of the lift curve
par(mar = c(8,5,2,1))
plot(1:10, prop_success_in_bin, xaxt = 'n', pch = 20, col = "blue3", cex = 2, xlab = "",ylab = "Proportion of True Success")
lines(1:10, prop_success_in_bin, col= "dodgerblue3")
axis(1,at = 1:10, labels = levels(pred_cut), cex = .7, )
title("Lift Curve: Bozeman Flights")

#Some Discussion Questions: 

# + What are we seeing here? 
# + The proportion of successes is going up  
